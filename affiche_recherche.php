...
try {
    ...
    if (isset($_GET['delete'])) {
        ...
        $deleteQuery = "DELETE FROM Recherche WHERE id = :id";
        $deleteStmt = $conn->prepare($deleteQuery);
        $deleteStmt->bindParam(':id', $deleteId);
        $deleteStmt->execute();
    }

    $sql = "SELECT R.id, R.id_festival, R.places, R.recherche_aller, R.recherche_retour, F.nom_festival 
    FROM Recherche R
    ...
    if ($stmt->rowCount() > 0) {
        ...
        echo "<tr><th class='sortable'>Festival</th><th class='sortable'>Places</th><th class='sortable'>Date Aller</th><th class='sortable'>Date Retour</th>";
        if ($isAdmin) {
            echo "<th>Action</th>";
        }
        echo "</tr>";
        ...
        while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
            ...
            if ($isAdmin) {
                ...
                echo "<a href=\"edit_trajet.php?id=" . $row["id"] . "\">Modifier</a></td>";
            }
            echo "</tr>";
        }
